...
pub const Arg = union(enum) {
    ...
    pub fn flagOf(self: Arg, flags: anytype) ?(FlagError!Kind) {
        const value = (self.parse(?[:0]const u8, flags, null) orelse return null) catch unreachable;
        return switch (self) {
            .shorts => .short,
            .long => if (value != null) error.UnexpectedValueForFlag else .long,
            else => return null,
        };
    }
    ...
    /// long flag strings, both *without* their leading `-`
    /// ,e.g. `.{ "help", 'h' }`
    pub fn valueOf(self: Arg, flags: anytype, args: anytype) ?[:0]const u8 {
    ...
    fn parseShortArg(R: type, sh: *Shorts, flag: u21, args: anytype) ParseError!R {
        switch (R) {
            ...
            []const u8, [:0]const u8 => {
                const value, const source = try peekShortArgValue(sh, args);
                if (source == .args) consumeNextValue(args);
                return value;
            },
            else => switch (@typeInfo(R)) {
                .int => {
                    const value, const source = try peekShortArgValue(sh, args);
                    const res = try std.fmt.parseInt(R, value, 0);
                    if (source == .args) consumeNextValue(args);
                    return res;
                },
                .float => {
                    const value, const source = try peekShortArgValue(sh, args);
                    const res = try std.fmt.parseFloat(R, value);
                    if (source == .args) consumeNextValue(args);
                    return res;
                },
                .optional => |o| {
                ...
                .@"enum" => {
                    const value, const source = try peekShortArgValue(sh, args);
                    const res = std.meta.stringToEnum(R, value) orelse return ParseError.InvalidEnumTag;
                    if (source == .args) consumeNextValue(args);
                    return res;
                },
                else => {
                    const value, const source = try peekShortArgValue(sh, args);
                    const res = try parseCustom(R, value);
                    if (source == .args) consumeNextValue(args);
                    return res;
                },
            },
    ...
    fn peekShortArgValue(sh: *const Shorts, args: anytype) !struct { [:0]const u8, enum { flag, args } } {
        const val = sh.peekValue();
        if (val.len > 0) {
            return .{ val, .flag };
        }
        if (peekNextValue(args)) |v| {
            return .{ v, .args };
        }
        return ParseError.MissingRequiredValue;
    }
    ...
    fn parseLongArg(R: type, long: Long, args: anytype) ParseError!R {
        switch (R) {
            ...
            []const u8, [:0]const u8 => {
                const value, const source = try peekLongArgValue(long, args);
                if (source == .args) consumeNextValue(args);
                return value;
            },
            else => switch (@typeInfo(R)) {
    ...
    fn peekLongArgValue(long: Long, args: anytype) !struct { [:0]const u8, enum { flag, args } } {
        if (long.value) |v| {
            return .{ v, .flag };
        }
        if (peekNextValue(args)) |v| {
    ...
    fn peekNextValue(args: anytype) ?[:0]const u8 {
        switch (@typeInfo(@TypeOf(args))) {
            .void, .null => return null,
            else => return args.peekAsValue(),
        }
    }

    fn consumeNextValue(args: anytype) void {
        switch (@typeInfo(@TypeOf(args))) {
            .void, .null => {},
            else => _ = args.nextAsValue(),
        }
    }
    ...
    /// Renders the argument as a textual representation of the argument,
    /// close to how it would have been provided as a command line argument.
    pub fn fmt(arg: *const Arg) std.fmt.Alt(*const Arg, print) {
        return .{ .data = arg };
    }

    /// Renders the argument if it is a short flag, otherwise print nothing.
    pub fn fmtShort(arg: *const Arg) std.fmt.Alt(*const Arg, printShort) {
        return .{ .data = arg };
    }

    /// Renders the argument if it is a short flag and matching the provided
    /// name, otehrwise print nothing.
    pub fn fmtShortNamed(arg: *const Arg, comptime name: u21) std.fmt.Alt(FmtShort(name), FmtShort(name).print) {
        return .{ .data = .{ .arg = arg } };
    }

    /// Renders the argument if it is a long flag, otherwise print nothing.
    pub fn fmtLong(arg: *const Arg) std.fmt.Alt(*const Arg, printLong) {
        return .{ .data = arg };
    }

    /// Renders the argument if it is a long flag and matching the provided
    /// name, otherwise print nothing.
    pub fn fmtLongNamed(arg: *const Arg, comptime name: []const u8) std.fmt.Alt(FmtLong(name), FmtLong(name).print) {
        return .{ .data = .{ .arg = arg } };
    }

    /// Renders the argument if it as a value, otherwise print nothing.
    pub fn fmtValue(arg: *const Arg) std.fmt.Alt(*const Arg, printValue) {
        return .{ .data = arg };
    }

    /// Renders the argument if it is the escape string, otherwise print nothing.
    pub fn fmtEscape(arg: *const Arg) std.fmt.Alt(*const Arg, printEscape) {
        return .{ .data = arg };
    }

    pub fn print(arg: *const Arg, writer: *std.io.Writer) !void {
        switch (arg.*) {
    ...
    pub fn printShort(arg: *const Arg, writer: *std.io.Writer) !void {
        // render name of short flag
        switch (arg.*) {
            .shorts => |s| try writer.writeAll(s.flags),
            else => {},
        }
    }

    pub fn printLong(arg: *const Arg, writer: *std.io.Writer) !void {
        // render name of long flag
        switch (arg.*) {
            .long => |l| try writer.writeAll(l.flag),
            else => {},
        }
    }

    pub fn printValue(arg: *const Arg, writer: *std.io.Writer) !void {
        // render value
        switch (arg.*) {
            .long => |l| if (l.value) |lv| try writer.writeAll(lv),
            .value => |v| try writer.writeAll(v),
            else => {},
        }
    }

    pub fn printEscape(arg: *const Arg, writer: *std.io.Writer) !void {
        // render escape
        switch (arg.*) {
            .escape => try writer.writeAll("--"),
            else => {},
    ...
    fn FmtShort(comptime name: u21) type {
        return struct {
            arg: *const Arg,

            pub fn print(self: @This(), writer: *std.io.Writer) !void {
                if (self.arg.valueOf(.{name}, null)) |value| {
    ...
    fn FmtLong(comptime name: []const u8) type {
        return struct {
            arg: *const Arg,
    ...
    test valueOf {
        ...
        try expectStr("foo", short_arg.valueOf(.{ 'v', "value" }, null).?);
        try expect(null, short_arg.valueOf(.{ 'h', "help" }, null));

        const short_arg_no_equal = Arg{ .shorts = .{ .flags = "vfoo" } };
        try expectStr("foo", short_arg_no_equal.valueOf(.{ 'v', "value" }, null).?);

        const short_arg_no_value = Arg{ .shorts = .{ .flags = "v" } };
        try expect(null, short_arg_no_value.valueOf(.{ 'v', "value" }, null));

        const long_arg = Arg{ .long = .{ .flag = "value", .value = "foo" } };
        try expect("foo", long_arg.valueOf(.{ 'v', "value" }, null).?);
        try expect(null, long_arg.valueOf(.{ 'h', "help" }, null));

        const long_arg_no_value = Arg{ .long = .{ .flag = "value", .value = null } };
        try expect(null, long_arg_no_value.valueOf(.{ 'v', "value" }, null));

        // runtime values
        var short_flag: u8 = 'v';
        var long_flag: []const u8 = "value";
        _ = .{ &short_flag, &long_flag };
        try expectStr("foo", short_arg.valueOf(.{ short_flag, long_flag }, null).?);
        try expectStr("foo", long_arg.valueOf(.{ short_flag, long_flag }, null).?);
    }
    ...
    test "parse not reading value on parse error" {
        const Enum = enum { @"42" };
        const Custom = struct {
            pub fn parse(_: []const u8) ParseError!@This() {
                return ParseError.InvalidValue;
            }
        };

        inline for (.{ &.{ "--flag", "poop" }, &.{ "-f", "poop" } }) |args_input| {
            var args = @import("args.zig").SliceArgs.init(args_input);

            inline for (.{ u32, f32, Enum, Custom }) |R| {
                args.reset();

                const arg = args.next().?;
                const res = arg.parse(R, .{ 'f', "flag" }, &args).?;
                try t.expect(std.meta.isError(res));
                try t.expectEqualStrings("poop", args.nextAsValue().?);
            }
        }
    }
    ...
    test fmt {
        ...
        try t.expectFmt("--verbose=true", "{f}", .{arg.fmt()});
        try t.expectFmt("verbose", "{f}", .{arg.fmtLong()});
        try t.expectFmt("", "{f}", .{arg.fmtShort()});
        try t.expectFmt("true", "{f}", .{arg.fmtValue()});
        try t.expectFmt("true", "{f}", .{arg.fmtLongNamed("verbose")});
    }

    test "render" {
        const value_arg = Arg{ .value = "42" };
        try t.expectFmt("42", "{f}", .{value_arg.fmt()});

        const short_arg = Arg{ .shorts = .{ .flags = "a42" } };
        try t.expectFmt("-a42", "{f}", .{short_arg.fmt()});

        const long_arg = Arg{ .long = .{ .flag = "a", .value = null } };
        try t.expectFmt("--a", "{f}", .{long_arg.fmt()});

        const long_arg_with_value = Arg{ .long = .{ .flag = "a", .value = "42" } };
        try t.expectFmt("--a=42", "{f}", .{long_arg_with_value.fmt()});

        const escape_arg = Arg{ .escape = {} };
        try t.expectFmt("--", "{f}", .{escape_arg.fmt()});
    }

    test "render short flag" {
        const value_arg = Arg{ .value = "42" };
        try t.expectFmt("", "{f}", .{value_arg.fmtShort()});

        const short_arg = Arg{ .shorts = .{ .flags = "a42" } };
        try t.expectFmt("a42", "{f}", .{short_arg.fmtShort()});

        const long_arg = Arg{ .long = .{ .flag = "a", .value = null } };
        try t.expectFmt("", "{f}", .{long_arg.fmtShort()});

        const long_arg_with_value = Arg{ .long = .{ .flag = "a", .value = "42" } };
        try t.expectFmt("", "{f}", .{long_arg_with_value.fmtShort()});

        const escape_arg = Arg{ .escape = {} };
        try t.expectFmt("", "{f}", .{escape_arg.fmtShort()});
    }

    test "render value of short flag" {
        const value_arg = Arg{ .value = "42" };
        try t.expectFmt("", "{f}", .{value_arg.fmtShortNamed('a')});

        const short_arg = Arg{ .shorts = .{ .flags = "a42" } };
        try t.expectFmt("42", "{f}", .{short_arg.fmtShortNamed('a')});

        const long_arg = Arg{ .long = .{ .flag = "a", .value = null } };
        try t.expectFmt("", "{f}", .{long_arg.fmtShortNamed('a')});

        const long_arg_with_value = Arg{ .long = .{ .flag = "a", .value = "42" } };
        try t.expectFmt("", "{f}", .{long_arg_with_value.fmtShortNamed('a')});

        const escape_arg = Arg{ .escape = {} };
        try t.expectFmt("", "{f}", .{escape_arg.fmtShortNamed('a')});
    }

    test "render long flag" {
        const value_arg = Arg{ .value = "42" };
        try t.expectFmt("", "{f}", .{value_arg.fmtLong()});

        const short_arg = Arg{ .shorts = .{ .flags = "a42" } };
        try t.expectFmt("", "{f}", .{short_arg.fmtLong()});

        const long_arg = Arg{ .long = .{ .flag = "a", .value = null } };
        try t.expectFmt("a", "{f}", .{long_arg.fmtLong()});

        const long_arg_with_value = Arg{ .long = .{ .flag = "a", .value = "42" } };
        try t.expectFmt("a", "{f}", .{long_arg_with_value.fmtLong()});

        const escape_arg = Arg{ .escape = {} };
        try t.expectFmt("", "{f}", .{escape_arg.fmtLong()});
    }

    test "render value of long flag" {
        const value_arg = Arg{ .value = "42" };
        try t.expectFmt("", "{f}", .{value_arg.fmtLongNamed("a")});

        const short_arg = Arg{ .shorts = .{ .flags = "a42" } };
        try t.expectFmt("", "{f}", .{short_arg.fmtLongNamed("a")});

        const long_arg = Arg{ .long = .{ .flag = "a", .value = null } };
        try t.expectFmt("", "{f}", .{long_arg.fmtLongNamed("a")});

        const long_arg_with_value = Arg{ .long = .{ .flag = "a", .value = "42" } };
        try t.expectFmt("42", "{f}", .{long_arg_with_value.fmtLongNamed("a")});

        const escape_arg = Arg{ .escape = {} };
        try t.expectFmt("", "{f}", .{escape_arg.fmtLongNamed("a")});
    }

    test "render value" {
        const value_arg = Arg{ .value = "42" };
        try t.expectFmt("42", "{f}", .{value_arg.fmtValue()});

        const short_arg = Arg{ .shorts = .{ .flags = "a42" } };
        try t.expectFmt("", "{f}", .{short_arg.fmtValue()});

        const long_arg = Arg{ .long = .{ .flag = "a", .value = null } };
        try t.expectFmt("", "{f}", .{long_arg.fmtValue()});

        const long_arg_with_value = Arg{ .long = .{ .flag = "a", .value = "42" } };
        try t.expectFmt("42", "{f}", .{long_arg_with_value.fmtValue()});

        const escape_arg = Arg{ .escape = {} };
        try t.expectFmt("", "{f}", .{escape_arg.fmtValue()});
    }

    test "render escape" {
        const value_arg = Arg{ .value = "42" };
        try t.expectFmt("", "{f}", .{value_arg.fmtEscape()});

        const short_arg = Arg{ .shorts = .{ .flags = "a42" } };
        try t.expectFmt("", "{f}", .{short_arg.fmtEscape()});

        const long_arg = Arg{ .long = .{ .flag = "a", .value = null } };
        try t.expectFmt("", "{f}", .{long_arg.fmtEscape()});

        const long_arg_with_value = Arg{ .long = .{ .flag = "a", .value = "42" } };
        try t.expectFmt("", "{f}", .{long_arg_with_value.fmtEscape()});

        const escape_arg = Arg{ .escape = {} };
        try t.expectFmt("--", "{f}", .{escape_arg.fmtEscape()});
    }
};
